 ...
 */
router.post('/payment', auth_1.verifyWebhookSignature, async (req, res) => {
    const body = req.body;
    ...
    if (Array.isArray(body.apply)) {
        ...
        for (const block of body.apply) {
            for (const tx of block.transactions ?? []) {
                ...
                if (!txHash)
                    continue;
                const events = tx.metadata?.receipt?.events ?? tx.events ?? [];
                ...
                let solutionIdFromContract = null;
                ...
                if (printEvent) {
                    ...
                    if (match)
                        solutionIdFromContract = match[1];
                }
                ...
                if (!solutionIdFromContract && metadata.kind?.data?.function_name === 'lock-stx') {
                    const args = metadata.kind?.data?.function_args ?? [];
                    const idArg = args.find((a) => a.name === 'solution-id');
                    if (idArg?.repr)
                        solutionIdFromContract = idArg.repr.replace(/^"|"$/g, '');
                }
                // For Wallet Activity events (legacy / direct transfers):
                const transferEvent = events.find((e) => e.event_type === 'fungible_token_asset' &&
                    e.asset?.asset_event_type === 'transfer' &&
                    e.asset?.recipient?.toLowerCase() === PLATFORM_WALLET);
                const amountMicro = transferEvent
    ...
    try {
        ...
        if (!agent)
            throw new Error('Agent not found after upsert');
