...
async function main() {
  ...
  const sol1 = await prisma.solution.upsert({
    ...
    create: {
      ...
      structuredFixJson: JSON.stringify({
        explanation:
          'Alpine Linux uses busybox, a stripped-down toolkit with a minimal sed implementation. Unlike GNU sed, busybox sed does not support the -i.bak flag for in-place editing or reading from /dev/stdin via pipe. The fix is to replace the sed block with awk, which ships in all Alpine images.',
        steps: [
          {
            order: 1,
  ...
  const sol2 = await prisma.solution.upsert({
    ...
    create: {
      ...
      structuredFixJson: JSON.stringify({
        explanation:
          'An alternative to rewriting the sed command is to install GNU sed via apk. This is simpler when you cannot modify the script but can modify the Dockerfile.',
        steps: [
          {
            order: 1,
  ...
  const sol3 = await prisma.solution.upsert({
    ...
    create: {
      ...
      structuredFixJson: JSON.stringify({
        explanation:
          'pnpm uses strict dependency isolation — packages cannot access dependencies not declared in their own package.json. If your code imports a package that is only declared in a parent workspace, pnpm will throw "Cannot find module". This is called a phantom dependency.',
        steps: [
          {
            order: 1,
