...
// ─── Server definition ───────────────────────────────────────────────────────
const server = new index_js_1.Server({ name: "quash-mcp-plugin", version: "1.0.0" }, { capabilities: { tools: {} } });
// ─── Tool list ────────────────────────────────────────────────────────────────
server.setRequestHandler(types_js_1.ListToolsRequestSchema, async () => ({
    tools: [
        ...
        },
        {
            ...
            description: "Unlock and retrieve the full Quash solution for a known solution_id. " +
        ...
        },
        {
            ...
            description: "Post an error as a live bounty on Quash when no solution exists. " +
        ...
        },
        {
            ...
            description: "Poll the status of a live Quash bounty by problem_id. " +
 ...
 */
async function broadcastUsdcxTransfer(privateKey, recipient, amount, network) {
    // Dynamic import to keep startup fast
    const { makeContractCall, broadcastTransaction, AnchorMode, FungibleConditionCode, makeStandardFungiblePostCondition, createAssetInfo } = await Promise.resolve().then(() => __importStar(require("@stacks/transactions")));
    const { StacksTestnet, StacksMainnet } = await Promise.resolve().then(() => __importStar(require("@stacks/network")));
    ...
    const txOptions = {
        ...
        functionArgs: [
            (await Promise.resolve().then(() => __importStar(require("@stacks/transactions")))).uintCV(microAmount),
            (await Promise.resolve().then(() => __importStar(require("@stacks/transactions")))).standardPrincipalCV((await Promise.resolve().then(() => __importStar(require("@stacks/transactions")))).getAddressFromPrivateKey(privateKey)),
            (await Promise.resolve().then(() => __importStar(require("@stacks/transactions")))).standardPrincipalCV(recipient),
            (await Promise.resolve().then(() => __importStar(require("@stacks/transactions")))).noneCV(),
        ],
    ...
    return result.txid ?? result.error ?? "unknown";
}
